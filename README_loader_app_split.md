...
- [1. 本次工作环境](#1-本次工作环境)
- [2. 从源参考工程拆分为 Loader + App](#2-从源参考工程拆分为-loader--app)
    - [2.1 工程结构拆分](#21-工程结构拆分)
    - [2.2 xSPI Flash 中的 App 固定分区](#22-xspi-flash-中的-app-固定分区)
    - [2.3 App manifest 机制](#23-app-manifest-机制)
    - [2.4 App 拷贝段和运行地址](#24-app-拷贝段和运行地址)
    - [2.5 App LDR BTCM 偏移](#25-app-ldr-btcm-偏移)
    - [2.6 SDRAM 初始化职责](#26-sdram-初始化职责)
- [3. 集成到一个 IAR `.eww` 工作区的优势](#3-集成到一个-iar-eww-工作区的优势)
- [4. App 单独调试功能和注意事项](#4-app-单独调试功能和注意事项)
    - [4.1 集成调试和单独调试使用不同 ICF](#41-集成调试和单独调试使用不同-icf)
    - [4.2 正常 Loader + App 模式](#42-正常-loader--app-模式)
    - [4.3 App 单独调试模式](#43-app-单独调试模式)
    - [4.4 App manifest 校验点](#44-app-manifest-校验点)
- [5. 当前关键文件](#5-当前关键文件)
- [6. 推荐验证清单](#6-推荐验证清单)
- [7. 当前可能存在的问题和限制](#7-当前可能存在的问题和限制)
- [8. 使用 RASC 生成代码的注意事项](#8-使用-rasc-生成代码的注意事项)
- [9. 参考文献](#9-参考文献)  在开始这个工程的发开之前，请务必先理解这个参考文献的内容
...
需要注意的是，App LDR 部分的代码仍然必须运行，Loader 不能在完成段拷贝后直接跳到 App `main()`。原因是 App LDR 承担的是 App 自身启动上下文的建立工作，包括 App 入口 `system_init`、启动阶段栈/运行环境准备、App 侧 `SystemInit()`/warm start 路径、BSP/中断/RTOS 进入 `main()` 前所需的初始化流程等。Loader 只能把 App image 放到正确运行地址，并提供跳转前的硬件基础条件；它不能替代 App 工程自己的启动入口。

Loader LDR 和 App LDR 的功能差异可以理解为：

| 项目 | Loader LDR | App LDR |
| --- | --- | --- |
| 运行时机 | 上电或复位后最先运行。 | Loader 完成 App 段拷贝后运行。 |
| 主要目标 | 建立最小启动环境，初始化必要硬件，读取/打包/搬运 App，并跳转到 App。 | 建立 App 自身运行上下文，执行 App 启动入口 `system_init`，进入 App runtime 和 `main()`。 |
| Flash/App 处理 | 负责读取 App manifest，按表把 App 各段复制到 ATCM/BTCM/System RAM/SDRAM/Non-cache 运行地址。 | 不负责从 Flash 打包或搬运整个 App；在集成模式下，App 段已经由 Loader 按 manifest 拷贝完成。 |
| 硬件初始化边界 | 负责跳 App 前必须完成的公共硬件前置初始化，例如 SDRAM 初始化。 | 负责 App 侧启动流程需要的 BSP/系统初始化；集成模式下应避免重复执行与 Loader 冲突的初始化。 |
| 跳转目标 | 最终跳转到 App manifest 中的 `entry_point`，当前为 App `system_init`。 | 完成 App 启动流程后进入 App `main()` 和 FreeRTOS/应用任务。 |

因此当前方案不是删除 App LDR，也不是绕过 App LDR，而是把 App LDR 运行地址从 Loader 使用的 BTCM 区域错开，让 Loader 可以安全地把 App LDR 拷贝到 BTCM 后再跳入 App `system_init`。
